...
impl From<(&AccountProxy, i64)> for AccountRecord {
    fn from((proxy, updated_at): (&AccountProxy, i64)) -> Self {
        Self{
            ...
            trading_pnl: proxy.trading_pnl(),
            position_pnl: proxy.position_pnl(),
            daily_pnl: proxy.daily_pnl(),
            transaction_fee: proxy.transaction_fee(),
...
impl From<(Uuid, &PositionProxy, i64)> for PositionRecord {
    fn from((account_id, proxy, updated_at): (Uuid, &PositionProxy, i64)) -> Self {
        Self{
            ...
            available_limited: proxy.available_limited(),
...
impl From<(Uuid, &PositionProxy, f64, i64)> for PositionRecord {
    fn from((account_id, proxy, last_price, updated_at): (Uuid, &PositionProxy, f64, i64)) -> Self {
        Self{
            account_id,
            code: proxy.code().to_string(),
            direction: proxy.direction(),
